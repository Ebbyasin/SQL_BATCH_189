...
SELECT calisan_id AS id FROM workers;--geçici olarak degistiriyor
SELECT calisan_id id FROM workers;--Bu da üsttekiyle aynı işe yarıyor,okunabilirlik açısından AS kullanılır.

SELECT calisan_id || calisan_isim FROM workers;--|| javadaki concatination islemini yapar
...
--2.yol-bu daha dinamik bir çözüm,bu da subquery ile elde edilir,sag taraf subquery,sol taraf outquery..
SELECT *
FROM calisanlar3
...
--calisanlar3 tablosunda ikinci en yüksek maasi alan calisani gösteriniz
...
FROM calisanlar3 
WHERE maas=(SELECT MAX(maas)
			FROM calisanlar3) OR maas=((SELECT MIN(maas)
			FROM calisanlar3))
...
FROM markalar--correlated subwuery

--SELECT DISTINCT(sehir) FROM calisanlar3;--sadece farkli olanları listeler
...
SELECT marka_isim,calisan_sayisi,(SELECT MAX(maas) FROM calisanlar3 WHERE isyeri=marka_isim) AS max_maas,
                                 (SELECT MIN(maas) FROM calisanlar3 WHERE isyeri=marka_isim) min_maas--AS kullanmadan da sadece max_maas yazarak isim vermis oluruz
FROM markalar;
...
--Mart ayinda 'Toyota' satisi yapılmıs ise Nisan ayindaki tüm ürünlerin bilgilerini listeleyiniz.

SELECT *
FROM nisan
WHERE EXISTS (SELECT urun_isim FROM mart WHERE urun_isim='Toyota')

--Mart ayinda 'Volvo' satisi yapılmıs ise Nisan ayindaki tüm ürünlerin bilgilerini listeleyiniz.
