...
SELECT isim,SUM (urun_miktar)--görmek istediğimiz her seyi select komutuyla yazarız
FROM manav
GROUP BY isim
...
SELECT isim,SUM (urun_miktar) toplam_kg--görmek istediğimiz her seyi select komutuyla yazarız
FROM manav
GROUP BY isim
...
--isme göre alinan toplam ürün miktarı ve ürün çeşit miktarını bulunuz.
